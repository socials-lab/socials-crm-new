-- Fix: cast selected_tier from text to service_tier enum in convert_lead_to_client
-- Error: "column "selected_tier" is of type service_tier but expression is of type text"
...
BEGIN
  ...
  -- Validate primary contact has email
  v_primary_email := NULLIF(TRIM(COALESCE(p_primary_contact->>'email', '')), '');
  IF v_primary_email IS NULL THEN
    RAISE EXCEPTION 'Primární kontakt musí mít vyplněný email'
      USING ERRCODE = 'check_violation';
  END IF;

  -- Validate email format
  IF NOT is_valid_email_format(v_primary_email) THEN
    RAISE EXCEPTION 'Email primárního kontaktu není ve správném formátu'
      USING ERRCODE = 'check_violation';
  END IF;

  -- 1. Validate IČO doesn't already exist
  IF EXISTS (SELECT 1 FROM clients WHERE ico = v_ico AND deleted_at IS NULL) THEN
  ...
  -- 4. Create client
  INSERT INTO clients (
    name, brand_name, ico, dic, website, country, industry, status, tier,
    sales_representative_id, billing_street, billing_city, billing_zip,
    billing_country, billing_email, acquisition_channel, start_date, end_date,
    notes, pinned_notes, fakturoid_subject_id, created_by
  ) VALUES (
    p_client_data->>'name',
  ...
  INSERT INTO engagements (
    ...
    monthly_fee, one_off_fee, currency, start_date, end_date,
    notice_period_months, offer_url, contract_url, notes
  ) VALUES (
    v_client_id,
  ...
  -- 8. Create engagement services (FIX: cast selected_tier to service_tier enum)
