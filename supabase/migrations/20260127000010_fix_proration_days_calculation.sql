-- Migration: Fix proration days calculation to use actual days in month
-- The previous migration hardcoded 30 days; this calculates actual days

-- Create helper function to get actual days in a month
CREATE OR REPLACE FUNCTION days_in_month(p_year INT, p_month INT)
RETURNS INT AS $$
BEGIN
  -- Get the last day of the month by going to first day of next month and subtracting 1 day
  RETURN EXTRACT(DAY FROM (DATE_TRUNC('month', MAKE_DATE(p_year, p_month, 1)) + INTERVAL '1 month' - INTERVAL '1 day'));
END;
$$ LANGUAGE plpgsql IMMUTABLE;

-- Update create_invoice_with_items to use calculated days instead of hardcoded 30
...
BEGIN
  -- Calculate actual days in the invoice month
...
GRANT EXECUTE ON FUNCTION days_in_month(INT, INT) TO authenticated;

-- Success message
DO $$
BEGIN
  RAISE NOTICE 'Fixed proration days calculation:';
  RAISE NOTICE '  - Created days_in_month() helper function';
  RAISE NOTICE '  - Updated create_invoice_with_items to calculate actual days per month';
  RAISE NOTICE '  - No more hardcoded 30 days';
END $$;
