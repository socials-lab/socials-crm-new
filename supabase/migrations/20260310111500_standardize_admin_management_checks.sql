-- Standardize admin/management checks to a single function.
-- Since is_admin_or_management() now includes super admins, all call sites
-- should use it directly to avoid role-check drift.
...
BEGIN
  ...
  FROM user_roles ur
  WHERE is_admin_or_management(ur.user_id)
