...
BEGIN
  ...
  allows_financial_sync := (
    ...
    AND (
      ...
      OR OLD.fakturoid_total_without_vat IS DISTINCT FROM NEW.fakturoid_total_without_vat
      OR OLD.fakturoid_total_with_vat IS DISTINCT FROM NEW.fakturoid_total_with_vat
      OR OLD.fakturoid_duzp_date IS DISTINCT FROM NEW.fakturoid_duzp_date
    )
  );
  ...
  IF (
    ...
    (OLD.fakturoid_total_without_vat IS DISTINCT FROM NEW.fakturoid_total_without_vat AND NOT allows_financial_sync) OR
    (OLD.fakturoid_total_with_vat IS DISTINCT FROM NEW.fakturoid_total_with_vat AND NOT allows_financial_sync) OR
    (OLD.fakturoid_duzp_date IS DISTINCT FROM NEW.fakturoid_duzp_date AND NOT allows_financial_sync) OR
    OLD.issued_by IS DISTINCT FROM NEW.issued_by
  ) THEN
    RAISE EXCEPTION 'Issued invoices cannot be modified. Allowed updates: fakturoid_id/url, status, paid_at, approved invoice number sync and approved financial sync (amount/currency/date/DUZP/net) from Fakturoid.';
  END IF;
