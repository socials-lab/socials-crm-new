...
-- Fix lead conversion crash when inserting non-Creative Boost services.
-- engagement_services.creative_boost_fixed_billing is NOT NULL, but the RPC
-- used to pass NULL for services without Creative Boost credits.
-- Keep the newer server-side authz checks while applying that defaulting fix.
...
BEGIN
  ...
  LOOP
    INSERT INTO engagement_services (
      ...
      COALESCE(
        NULLIF(v_service->>'creative_boost_fixed_billing', '')::BOOLEAN,
        CASE WHEN NULLIF(v_service->>'creative_boost_credits', '') IS NOT NULL THEN true ELSE false END
      )
