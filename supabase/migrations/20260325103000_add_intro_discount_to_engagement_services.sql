-- Intro discount support for lead services converted into engagements

ALTER TABLE engagement_services
  ADD COLUMN IF NOT EXISTS intro_discount_percent DECIMAL(5,2),
  ADD COLUMN IF NOT EXISTS intro_discount_months INTEGER,
  ADD COLUMN IF NOT EXISTS intro_discount_start_date DATE;

ALTER TABLE engagement_services
  DROP CONSTRAINT IF EXISTS engagement_services_intro_discount_percent_range,
  ADD CONSTRAINT engagement_services_intro_discount_percent_range
    CHECK (intro_discount_percent IS NULL OR (intro_discount_percent > 0 AND intro_discount_percent <= 100));

ALTER TABLE engagement_services
  DROP CONSTRAINT IF EXISTS engagement_services_intro_discount_months_range,
  ADD CONSTRAINT engagement_services_intro_discount_months_range
    CHECK (intro_discount_months IS NULL OR (intro_discount_months > 0 AND intro_discount_months <= 24));

ALTER TABLE engagement_services
  DROP CONSTRAINT IF EXISTS engagement_services_intro_discount_consistency,
  ADD CONSTRAINT engagement_services_intro_discount_consistency
    CHECK (
      (intro_discount_percent IS NULL AND intro_discount_months IS NULL AND intro_discount_start_date IS NULL)
      OR
      (intro_discount_percent IS NOT NULL AND intro_discount_months IS NOT NULL AND intro_discount_start_date IS NOT NULL)
    );
