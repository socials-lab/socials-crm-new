-- Migration: Create stored procedure for atomic lead-to-client conversion
-- This ensures all database operations happen in a single transaction
...
BEGIN
  ...
  INSERT INTO clients (
    ...
    COALESCE(p_client_data->>'main_contact_name', ''),
    COALESCE(p_client_data->>'main_contact_email', ''),
    COALESCE(p_client_data->>'main_contact_phone', ''),
  ...
  LOOP
    INSERT INTO engagement_services (
      ...
      CASE
        WHEN v_service->>'billing_type' = 'one_off' THEN 'pending'::invoicing_status
        ELSE 'not_applicable'::invoicing_status
      END
...
-- Add helpful comment
COMMENT ON FUNCTION convert_lead_to_client IS
'Atomically converts a lead to a client with engagement, contacts, services, and assignments.
All operations happen in a single transaction - if any step fails, everything is rolled back.

Parameters:
- p_lead_id: The lead to convert
- p_client_data: Client fields (name, ico, dic, billing info, etc.)
- p_primary_contact: Primary contact info (name, email, phone, position)
- p_additional_contacts: Array of additional contacts
- p_engagement_data: Engagement fields (name, type, fees, dates)
- p_services: Array of services to create
- p_assignments: Array of team member assignments

Returns JSONB with client_id, engagement_id, and primary_contact_id on success.
Raises exception on failure, automatically rolling back all changes.';
