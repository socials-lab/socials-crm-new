-- Hotfix: align missing line-item currency validation with invoice currency
-- Previous RPC validation defaulted missing line currency to CZK, which incorrectly
-- rejected non-CZK invoices when any line item omitted currency.
...
BEGIN
  ...
  LOOP
    -- Keep validation consistent with insertion/defaulting logic.
    v_line_currency := COALESCE(v_line_item->>'currency', p_currency);
