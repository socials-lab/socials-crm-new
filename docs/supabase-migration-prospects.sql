...
CREATE POLICY "CRM users can read prospects" ON public.prospects FOR SELECT TO authenticated USING (has_crm_access(auth.uid()));
CREATE POLICY "CRM users can manage prospects" ON public.prospects FOR ALL TO authenticated USING (has_crm_access(auth.uid())) WITH CHECK (has_crm_access(auth.uid()));

CREATE POLICY "CRM users can read prospect_interactions" ON public.prospect_interactions FOR SELECT TO authenticated USING (has_crm_access(auth.uid()));
CREATE POLICY "CRM users can manage prospect_interactions" ON public.prospect_interactions FOR ALL TO authenticated USING (has_crm_access(auth.uid())) WITH CHECK (has_crm_access(auth.uid()));
