...
export function TeamInvoicingOverview() {
  ...
  const [selectedColleague, setSelectedColleague] = useState<ColleagueInvoiceData | null>(null);
  ...
  const colleagueInvoices = useMemo(() => {
    ...
    return activeColleagues.map(colleague => {
      // 1. Client assignments (fixed monthly)
      ...
      const creativeBoostItems = creditsByClient.map(cb => ({
        name: cb.clientName,
      ...
      // 4. Extra work
      const colleagueExtraWorks = extraWorks.filter(ew => {
      ...
      const extraWorkItems = colleagueExtraWorks.map(ew => {
      ...
      // 5. Manual items (from localStorage)
      ...
      const clientTotal = clientItems.reduce((s, i) => s + i.amount, 0)
        + creativeBoostItems.reduce((s, i) => s + i.amount, 0)
        + commissionItems.reduce((s, i) => s + i.amount, 0)
        + extraWorkItems.reduce((s, i) => s + i.amount, 0);
      
      const internalTotal = manualItems
        .filter(i => i.category === 'marketing' || i.category === 'overhead')
        .reduce((s, i) => s + i.amount, 0);
      
      const clientWorkManual = manualItems
        .filter(i => i.category === 'client_work')
        .reduce((s, i) => s + i.amount, 0);

      const grandTotal = clientTotal + clientWorkManual + internalTotal;
      ...
      return {
        ...
        extraWorkItems,
        manualItems,
        clientTotal: clientTotal + clientWorkManual,
        internalTotal,
        grandTotal,
        itemCount,
      };
  ...
  const handleViewDetail = (data: typeof colleagueInvoices[0]) => {
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold flex items-center gap-2">
            ...
            Výkazy kolegů – {MONTHS[selectedMonth - 1]} {selectedYear}
          </CardTitle>
        </CardHeader>
