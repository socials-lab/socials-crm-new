...
import { TrendingUp, FileText, Briefcase, AlertTriangle } from 'lucide-react';
...
export function ExtraWorkMarginSection({ year, month }: ExtraWorkMarginSectionProps) {
  ...
  const extraWorkMarginData = useMemo(() => {
    ...
    const extraWorksWithMargin = monthExtraWorks.map((ew) => {
      ...
      if (ew.hours_worked && !colleague?.internal_hourly_cost) {
        return {
          id: ew.id,
          name: ew.name,
          ...
          dataIssue: 'Chybí internal_hourly_cost u kolegy.',
        };
      }
      ...
      if (!ew.hours_worked) {
        return {
          ...
          status: ew.status,
          hoursWorked: ew.hours_worked,
          dataIssue: 'Chybí hours_worked, nelze spočítat marži.',
        };
      }

      const hourlyRate = colleague?.internal_hourly_cost;
      const cost = ew.hours_worked * hourlyRate;
      ...
      return {
        ...
        status: ew.status,
        hoursWorked: ew.hours_worked,
        dataIssue: null as string | null,
      };
    });

    const calculable = extraWorksWithMargin.filter((item) => item.margin !== null);
    const totalRevenue = calculable.reduce((sum, item) => sum + item.amount, 0);
    const totalCost = calculable.reduce((sum, item) => sum + (item.cost || 0), 0);
  ...
  const extraWorkMarginTrend = useMemo(() => {
    return Array.from({ length: 12 }, (_, i) => {
      ...
      monthExtraWorks.forEach((ew) => {
        ...
        const ewCost = ew.hours_worked * colleague.internal_hourly_cost;
        revenue += ew.amount;
        cost += ewCost;
      });
  ...
  function getMarginBadgeColor(percent: number) {
  ...
  return (
    <div className="space-y-6">
      {extraWorkMarginData.issueCount > 0 && (
        <Card className="border-amber-400/40 bg-amber-500/5">
          <CardContent className="pt-6 flex items-center gap-2 text-sm">
            <AlertTriangle className="h-4 w-4 text-amber-600" />
            <span>
              U {extraWorkMarginData.issueCount} víceprací chybí data pro výpočet marže.
            </span>
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground">Počet (spočítáno)</p>
                  ...
                  {extraWorkMarginData.extraWorks.slice(0, 15).map((ew) => (
                    <TableRow key={ew.id}>
                      ...
                      <TableCell className="text-right text-muted-foreground">
                        {ew.cost === null ? '—' : formatCurrency(ew.cost)}
                      </TableCell>
                      <TableCell className="text-right font-medium">
                        {ew.margin === null ? '—' : formatCurrency(ew.margin)}
                      </TableCell>
                      <TableCell className="text-right">
                        {ew.marginPercent === null ? (
                          <Badge variant="destructive">Chybí data</Badge>
                        ) : (
