...
export function TeamCapacityForecast({
  ...
  const { knownColleagues, unknownSlotColleagues } = useMemo(() => {
    const activeColleagues = colleagues.filter((c) => c.status === 'active');
    const known: Array<{ colleague: Colleague; primarySlotType: ServiceSlotType }> = [];
    const unknown: Colleague[] = [];

    activeColleagues.forEach((colleague) => {
      const type = getSlotTypeFromPosition(colleague.position);
      if (!type) {
        unknown.push(colleague);
        return;
      }
      known.push({ colleague, primarySlotType: type });
    });

    return { knownColleagues: known, unknownSlotColleagues: unknown };
  }, [colleagues]);

  const colleagueCapacity = useMemo(() => {
    return knownColleagues.map(({ colleague, primarySlotType }) => {
  ...
  }, [knownColleagues, assignments, engagements, plannedForMonth, monthStart]);
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          {colleagueCapacity.length > 6 && (
            <Button variant="ghost" size="sm" onClick={() => setShowAllColleagues(!showAllColleagues)}>
      ...
      <CardContent>
        {unknownSlotColleagues.length > 0 && (
          <div className="mb-4 p-3 rounded-lg border border-destructive/40 bg-destructive/5">
            <div className="flex items-start gap-2">
              <AlertTriangle className="h-4 w-4 text-destructive mt-0.5" />
              <div className="text-sm">
                <p className="font-medium text-destructive">Neznámé pozice bez mapování kapacity</p>
                <p className="text-muted-foreground mt-1">
                  Doplň mapování v `getSlotTypeFromPosition` pro:
                  {' '}
                  {unknownSlotColleagues.map((c) => `${c.full_name} (${c.position})`).join(', ')}.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-3 mb-4 pb-3 border-b">
          {SERVICE_SLOT_TYPES.map((slotType) => (
            <div key={slotType} className={`flex items-center gap-2 px-3 py-1.5 rounded-full ${SERVICE_SLOT_BG_COLORS[slotType]}`}>
              <span className={`text-sm font-medium ${SERVICE_SLOT_TEXT_COLORS[slotType]}`}>{SERVICE_SLOT_LABELS[slotType]}</span>
              <span className={`text-sm ${SERVICE_SLOT_TEXT_COLORS[slotType]}`}>{totalFreeCapacityByType[slotType]} volných</span>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {displayedColleagues.map(({ colleague, slots, slotCounts, primarySlotType, events, hasChanges }) => (
            <div key={colleague.id} className={`p-3 rounded-lg border ${hasChanges ? 'bg-muted/50' : ''}`}>
                ...
                {SERVICE_SLOT_TYPES.filter((st) => slots[st] > 0).map((slotType) => {
                  ...
                  return (
                    <div key={slotType} className="flex items-center gap-2">
                      <span className={`text-xs w-16 ${SERVICE_SLOT_TEXT_COLORS[slotType]}`}>{SERVICE_SLOT_LABELS[slotType]}</span>
                      <Progress value={(afterNew / max) * 100} className="h-1.5 flex-1" />
                      <span className="text-xs text-muted-foreground w-12 text-right">
                        {changed ? `${current}->${afterNew}/${max}` : `${current}/${max}`}
                      </span>
                    </div>
                  );
                })}
