...
export function AssignmentForm({
  ...
  function handleRoleChange(role: string) {
    ...
    if (isPerCreditRole(role)) {
      form.setValue('cost_model', 'per_credit', { shouldValidate: true, shouldDirty: true });
      return;
    }
    if (!isPerCreditRole(role) && form.getValues('cost_model') === 'per_credit') {
    ...
    try {
      ...
      await Promise.race([
        onSubmit({
          ...
          reward_per_credit:
            data.cost_model === 'per_credit'
              ? (data.reward_per_credit_banner ?? DEFAULT_BANNER_REWARD_PER_CREDIT)
              : null,
          ...
          reward_per_credit_video:
            data.cost_model === 'per_credit'
              ? (data.reward_per_credit_video ?? DEFAULT_VIDEO_REWARD_PER_CREDIT)
              : null,
