...
const portfolioTable = () => supabase.from('portfolio_items' as never);
...
export function usePortfolioData() {
  ...
  const deleteItem = async (item: PortfolioItem) => {
    ...
    if (markerIndex === -1) {
      toast({ title: 'Chyba při mazání', description: 'Neplatná URL souboru v portfoliu', variant: 'destructive' });
      return;
    }
    const objectPath = decodeURIComponent(item.file_url.slice(markerIndex + marker.length));
