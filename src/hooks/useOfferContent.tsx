...
export function useOfferContent() {
  ...
  interface OfferContentBlockRow {
    id: string;
    section_key: string;
    title: string | null;
    subtitle: string | null;
  ...
  const fetchBlocks = useCallback(async () => {
    try {
      ...
      (data as unknown as OfferContentBlockRow[])?.forEach((row) => {
        map[row.section_key] = row;
      });
      setBlocks(map);
    } catch (error) {
      console.error('Failed to load offer content blocks:', error);
      toast.error('Nepodarilo se nacist obsah nabidky z databaze');
      throw error;
    } finally {
      setIsLoading(false);
    }
  ...
  const updateBlock = useCallback(
    async (sectionKey: string, updates: { title?: string | null; subtitle?: string | null; content?: Record<string, unknown> }) => {
